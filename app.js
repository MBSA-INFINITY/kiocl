...
function runInput(arrayInput) {
    ...
    var _a;
    const input = (_a = arrayInput) === null || _a === void 0 ? void 0 : _a.toString().replace(/\s/g, "");
...
//    highlightValue(Math.floor(Math.random() * 6) + 1)
// },1000)

var objects = {"hammers": 1, "m10 screws":2, "m6 screws":3,"dampers": 4, "engines": 5, "helmets": 6, "mbsa": 7}
document.getElementById("material").addEventListener("input",(event)=>{
  let input_material = event.target.value
  for(const key in objects){
    // console.log(key)
    if(key.includes(input_material.toLowerCase())){
      highlightValue(objects[key])
    }
  }
